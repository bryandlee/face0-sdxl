# From: https://github.com/bmaltais/kohya_ss/blob/master/library/sdxl_lpw_stable_diffusion.py
...
def get_hidden_states(text_encoder, input_ids, is_sdxl_text_encoder2: bool, device):
    if not is_sdxl_text_encoder2:
        ...
        enc_out = text_encoder(
            input_ids.to(text_encoder.device), output_hidden_states=True, return_dict=True
        )
...
def get_unweighted_text_embeddings(
    ...
    if max_embeddings_multiples > 1:
        ...
        text_embeddings, text_pool = get_hidden_states(
            pipe.text_encoder, text_input, is_sdxl_text_encoder2, pipe.device
        )
...
def get_additional_embeddings(original_size, crop_coords_top_left, target_size):
...
class SdxlStableDiffusionLongPromptWeightingPipeline:
    ...
    def to(self, device=None, dtype=None):
        ...
        self.text_encoders[0].to(device=self.device, dtype=self.dtype)
        self.text_encoders[1].to(device=self.device, dtype=self.dtype)
    ...
    def prepare_latents(
        self, image, timestep, batch_size, height, width, dtype, device, generator, latents=None
    ):
        if image is None:
            shape = (
                batch_size,
            ...
            init_latent_dist = self.vae.encode(image.to(dtype=self.vae.dtype)).latent_dist
            init_latents = init_latent_dist.sample(generator=generator)
            init_latents = init_latents * self.vae.config.scaling_factor
            init_latents = torch.cat([init_latents] * batch_size, dim=0).to(dtype=dtype)
    ...
    def __call__(
        ...
        face_embeddings: Optional[torch.FloatTensor] = None,
    ):
        r"""
        ...
        size_embeddings = get_additional_embeddings([height, width], [0, 0], [height, width]).to(
        ...
        if do_classifier_free_guidance:
            ...
            if face_embeddings is not None:
                text_embeddings[:, -face_embeddings.shape[1] :] = face_embeddings
            ...
            additional_embeddings = {
                "time_ids": size_embeddings.repeat(2, 1).to(dtype),
                "text_embeds": torch.cat([uncond_pool, text_pool], dim=0).to(dtype),
            }
        else:
            text_embedding = torch.cat(text_embeddings_list, dim=2).to(dtype)
            if face_embeddings is not None:
                text_embedding[:, -face_embeddings.shape[1] :] = face_embeddings
            additional_embeddings = {
                "time_ids": size_embeddings,
                "text_embeds": text_pool.to(dtype),
            }
        ...
        for i, t in enumerate(self.progress_bar(timesteps)):
            ...
            noise_pred = self.unet(
                latent_model_input, t, text_embedding, added_cond_kwargs=additional_embeddings
            ).sample
